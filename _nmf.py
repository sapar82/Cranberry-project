...
from update_rules import _update_w_fast, _update_h_fast
...
def _beta_divergence(X, W, H, beta_loss, square_root=False):
    ...
    # beta-divergence, beta not in (0, 1, 2)
        ...
        if sp.issparse(X):
            ...
            for i in range(X.shape[1]):
                sum_WH_beta += np.sum(np.dot(W, H[:, i]) ** beta_loss)

        else:
            sum_WH_beta = np.sum(WH ** beta_loss)

        sum_X_WH = np.dot(X_data, WH_data ** (beta_loss - 1))
        res = (X_data ** beta_loss).sum() - beta_loss * sum_X_WH
        res += sum_WH_beta * (beta_loss - 1)
        res /= beta_loss * (beta_loss - 1)
...
def _prior_divergence(H, Hp, beta_loss, square_root=False):
    """Compute the beta-divergence of X and dot(W, H).
...
def _update_coordinate_descent(X, W, Ht, Hpt, l1_reg, l2_reg, shuffle,
                               random_state, beta, update_H):
    ...
    if not update_H:
        return _update_w_fast(W, HHt, XHt)
    else:
        return _update_h_fast(W, HHt, XHt, Hpt, beta)


def _fit_coordinate_descent(X, W, H, Hp, tol=1e-4, max_iter=200, l1_reg_W=0,
                            l1_reg_H=0, l2_reg_W=0, l2_reg_H=0, update_H=True,
                            verbose=0, shuffle=False, random_state=None, beta=0.):
    ...
    """
    # so W, Ht and Hpt are in C order in memory
    Ht = check_array(H.T, order='C')
    Hpt = check_array(Hp.T, order='C')
    ...
    for n_iter in range(1, max_iter + 1):
        ...
        # Update W
        violation += _update_coordinate_descent(X, W, Ht, Hpt, l1_reg_W,
                                                l2_reg_W, shuffle, rng, beta, update_H=False)
        # Update H
        if update_H:
            violation += _update_coordinate_descent(X.T, Ht, W, Hpt, l1_reg_H,
                                                    l2_reg_H, shuffle, rng, beta, update_H=True)
...
def non_negative_factorization(X, W=None, H=None, Hp=None, n_components=None, *,
                               init='warn', update_H=True, solver='cd',
                               beta_loss='frobenius', tol=1e-4,
                               max_iter=200, alpha=0., l1_ratio=0.,
                               regularization=None, random_state=None,
                               verbose=0, shuffle=False, beta=0.):
    ...
    if solver == 'cd':
        W, H, n_iter = _fit_coordinate_descent(X, W, H, Hp, tol, max_iter,
                                               l1_reg_W, l1_reg_H,
                                               l2_reg_W, l2_reg_H,
                                               update_H=update_H,
                                               verbose=verbose,
...
class NMF(TransformerMixin, BaseEstimator):
    ...
    def __init__(self, n_components=None, *, init='warn', solver='cd',
                 ...
                 shuffle=False, regularization='both', beta=0.):
    ...
    def fit_transform(self, X, y=None, W=None, H=None, Hp=None):
        ...
        with config_context(assume_finite=True):
            W, H, n_iter_ = non_negative_factorization(
                X=X, W=W, H=H, Hp=Hp, n_components=self.n_components, init=self.init,
                update_H=True, solver=self.solver, beta_loss=self.beta_loss,
                tol=self.tol, max_iter=self.max_iter, alpha=self.alpha,
                l1_ratio=self.l1_ratio, regularization=self.regularization,
                random_state=self.random_state, verbose=self.verbose,
                shuffle=self.shuffle, beta=self.beta)

        self.reconstruction_err_ = _beta_divergence(X, W, H, self.beta_loss,
                                                    square_root=True)
        self.divergence_err_ = _prior_divergence(H, Hp, self.beta_loss,
                                                square_root=True)
